/* Rent Tracker — shell + views (loaded after rent-charts.jsx). */
...
/* ---------------- Houses ---------------- */
function Houses({ house, onAddRent, onAddHouse }) {
      ...
      <RCard padding="0" style={{ overflow: 'hidden' }}>
        {house.rooms.map((r, i) => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px var(--card-pad)',
            ...
            <div className="ps-mono" style={{ width: 80, textAlign: 'right', fontSize: 'var(--text-sm)', fontWeight: 600,
              color: r.paid ? 'var(--text-heading)' : 'var(--text-faint)' }}>{r.paid ? gbp(r.paid) : '—'}</div>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <RIcon label="Edit room" variant="ghost" size="sm">{di('pencil')}</RIcon>
              ...
              {isOpen && (
                <div style={{ padding: '0 var(--card-pad) 12px' }}>
                  {RR.MONTHS.slice(0, 6).map((m, mi) => (
                    <div key={m} onClick={() => toast('Open ' + c.label + ' · ' + m)} style={{ display: 'flex', alignItems: 'center', gap: 10,
                      padding: '8px 10px', cursor: 'pointer', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-sm)' }}
                      onMouseEnter={e => e.currentTarget.style.background = 'var(--surface-hover)'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                      <span style={{ width: 36, color: 'var(--text-muted)' }}>{m}</span>
                      <span className="ps-mono" style={{ flex: 1, color: 'var(--text-body)' }}>{gbp(RR.EXP_MONTH[c.id] - mi * 7)}</span>
                      {mi % 2 === 0 && <RBadge tone="neutral" size="sm" ><span style={{ display: 'inline-flex', width: 12, height: 12, marginRight: 3 }}><i data-lucide="paperclip"></i></span>Receipt</RBadge>}
                    </div>
                  ))}
                </div>
              )}
    ...
    <div className="ps-fade">
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>{RR.RECEIPTS.length} receipts on file</div>
        </div>
        <RBtn variant="primary" leadingIcon={di('upload')} onClick={() => toast('Upload receipt')}>Upload receipt</RBtn>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 14 }}>
        {RR.RECEIPTS.map(rc => {
          const c = catById[rc.cat];
          return (
            <RCard key={rc.id} interactive padding="0" style={{ overflow: 'hidden', cursor: 'pointer' }} onClick={() => toast(rc.merchant + ' receipt')}>
...
function RentApp() {
  ...
  const [userOpen, setUserOpen] = useS(false);
  const [rentRoom, setRentRoom] = useS(null);
  ...
  const house = RR.HOUSES.find(h => h.id === houseId);
  const roomCount = RR.HOUSES.reduce((s, h) => s + h.rooms.length, 0);
  ...
  const sidebar = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '14px 18px', borderTop: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>{RR.HOUSES.length} houses · {roomCount} rooms</span>
  ...
  return (
    <ResponsiveShell sidebar={sidebar} topBar={topBar}>
      ...
      {view === 'houses' && <Houses house={house} onAddRent={setRentRoom} onAddHouse={() => toast('Add house wizard')} />}
      {view === 'expenses' && <Expenses toast={toast} />}
      {view === 'receipts' && <Receipts toast={toast} />}

      <AddRentDrawer room={rentRoom} onClose={() => setRentRoom(null)} onSave={() => { setRentRoom(null); toast('Rent recorded'); }} />
    </ResponsiveShell>
  );
}
...
function RentNav({ n, active, onClick }) {
  ...
  return <RD.NavItem icon={di(n.icon)} label={n.label} active={active} onClick={() => { onClick(n.id); close(); }} />;
}

function AddRentDrawer({ room, onClose, onSave }) {
  const [name, setName] = useS('');
  const [due, setDue] = useS('');
  const [recv, setRecv] = useS('');
  useE(() => { if (room) { setName(room.tenant || ''); setDue(String(room.rent)); setRecv(room.paid ? String(room.paid) : ''); } }, [room]);
  return (
    <RightDrawer open={!!room} onClose={onClose} title={room ? 'Add rent · ' + room.unit : ''} subtitle="Record a rent payment"
      footer={<><RBtn variant="secondary" onClick={onClose}>Cancel</RBtn><RBtn variant="primary" onClick={onSave}>Save</RBtn></>}>
      {room && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <RInput label="Renter name" value={name} onChange={e => setName(e.target.value)} placeholder="Tenant name" />
          <RInput label="Amount due" value={due} onChange={e => setDue(e.target.value)} leadingIcon={<span style={{ fontWeight: 700 }}>£</span>} />
          <RInput label="Amount received" value={recv} onChange={e => setRecv(e.target.value)} leadingIcon={<span style={{ fontWeight: 700 }}>£</span>} />
          <div style={{ display: 'flex', gap: 10 }}>
            <RBtn variant="secondary" size="sm" onClick={() => setRecv(due)}>Mark paid in full</RBtn>
            <RBtn variant="ghost" size="sm" onClick={() => setRecv('')}>Clear</RBtn>
          </div>
        </div>
      )}
    </RightDrawer>
