          ...
          <section key={p.id} style={{ marginBottom: 26 }}>
            ...
            <TCard>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                  ...
                  background: 'var(--surface-sunken)', color: 'var(--text-muted)' }}>{di('user-plus')}</span>
                  ...
                  <TBtn variant="secondary" size="sm" leadingIcon={di('building-2')}
                  onClick={() => {window.location.href = 'Houses.html';}}>Update in Houses</TBtn>
                </div>
              </TCard> :
...
function TenantApp() {
  ...
  const [, setTick] = useT(0); // bump to re-render when TX.PROPS/TENANTS change in place
  ...
  useTE(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      window.TENANT.fromStore();
      setUser(u);setAuth('in');
      unsub = window.PS_STORE.subscribe(() => {window.TENANT.fromStore();setTick((n) => n + 1);});
    })();
  ...
  // Resolve the active tenant defensively: the stored default id may not exist
  // in a freshly-populated portfolio (or one whose houses have no tenants yet).
  const tenant = TX.TENANTS.find((t) => t.id === tenantId) || TX.TENANTS[0] || null;
  const activeTid = tenant ? tenant.id : null;
  const tenantSuggs = suggs.filter((s) => s.tenant === activeTid);
  const signOut = () => {window.PS.Auth.signOut().finally(() => {window.location.href = 'Property Suite.html';});};

  const sendMsg = (msg) => activeTid && setThreads((th) => ({ ...th, [activeTid]: [...(th[activeTid] || []), msg] }));
  ...
  const updateContact = (id, patch) => {
    const t = TX.TENANTS.find((x) => x.id === id);if (t) Object.assign(t, patch); // optimistic in-memory (our own write is echo-ignored by the store)
    window.PS_STORE.updateTenant(id, patch);
    setTick((n) => n + 1);
  ...
  <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '18px 16px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <a href="Property Suite.html" title="Back to launcher" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          ...
          <span style={{ fontSize: 'var(--text-md)', fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--text-heading)' }}>Tenant Bridge</span>
        </a>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 12px' }}>
        {TABS.map((t) => {
        let badge;
        if (t.id === 'ai') badge = suggs.length || undefined;else
        if (t.id === 'queue') badge = queue.length || undefined;
        return (
  ...
  <div className="ps-topbar">
      <Hamburger />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="eyebrow">{view === 'thread' && tenant ? (TX.PROPS.find((p) => p.id === tenant.prop)?.name || 'TenantBridge') : 'TenantBridge'}</div>
        <h1 style={{ margin: 0, fontSize: 'var(--text-xl)', fontWeight: 700, letterSpacing: '-0.015em', color: 'var(--text-heading)' }}>
          {view === 'thread' ? (tenant ? tenant.name : 'Thread') : tabMeta.label}
        </h1>
      </div>
  ...
  return (
    ...
    innerStyle={fill ? { height: '100%', display: 'flex', flexDirection: 'column', padding: '20px 24px 24px', maxWidth: 'none' } : undefined}>
      <div style={fill ? { flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 } : {}}>
        ...
        {view === 'thread' && (tenant ?
        <ThreadView tenant={tenant} msgs={threads[activeTid] || []}
        suggestions={tenantSuggs} onSend={sendMsg} onApprove={approveSugg}
        onDismiss={dismissSugg} onEditDraft={editDraft} onUpdateContact={updateContact} /> :
        <TCard><EmptyState icon="users" title="No tenants yet" body="Add a tenant to a property to start a conversation thread." /></TCard>)}
        {view === 'ai' && <AIAssistant />}
