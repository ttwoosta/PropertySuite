      ...
      <div className="ps-composer" style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 14, background: 'var(--surface-card)',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10 }}>
          ...
          <div style={{ position: 'relative', flex: 'none' }}>
              {chanOpen &&
            <React.Fragment>
                  <div onClick={() => setChanOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 20 }} />
                  <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: 0, zIndex: 21, display: 'flex', flexDirection: 'column',
                minWidth: 150, padding: 5, gap: 2, background: 'var(--surface-card)', border: '1px solid var(--border-default)',
                borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-md)' }}>
                    {CHAN.map((c) => {
                  const on = channel === c.id;
                  return (
                    <button key={c.id} onClick={() => {onChannel(c.id);setChanOpen(false);}}
                    style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', borderRadius: 'var(--radius-sm)',
                      fontSize: 'var(--text-sm)', fontWeight: 600, cursor: 'pointer', border: 'none', textAlign: 'left',
                      background: on ? 'var(--brand-tint)' : 'transparent',
                      color: on ? 'var(--brand-on-tint)' : 'var(--text-heading)' }}>
                          <span style={{ display: 'inline-flex', width: 15, height: 15 }}><i data-lucide={c.icon}></i></span>
                          {c.label}
                        </button>);
              ...
              <button onClick={() => setChanOpen((o) => !o)} aria-label={'Channel: ' + cur.label} style={{ display: 'flex', alignItems: 'center', gap: 6,
              height: 38, padding: '0 11px', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-xs)', fontWeight: 600,
              cursor: 'pointer', border: '1px solid ' + (chanOpen ? 'var(--brand)' : 'var(--border-default)'),
              background: chanOpen ? 'var(--brand-tint)' : 'var(--surface-card)',
              color: chanOpen ? 'var(--brand-on-tint)' : 'var(--text-muted)' }}>
                <span style={{ display: 'inline-flex', width: 14, height: 14 }}><i data-lucide={cur.icon}></i></span>
                {cur.label}
                <span style={{ display: 'inline-flex', width: 13, height: 13, marginLeft: -1 }}><i data-lucide="chevron-down"></i></span>
              </button>
            </div>
          }
      ...
      <div className="ps-fade" style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
        ...
        <div className="ps-fade" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
            ...
            <div key={r.l} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '10px 0', borderTop: '1px solid var(--border-subtle)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', fontWeight: 600, padding: "0px 0px 5px" }}>{r.l}</div>
                    <div style={{ fontSize: 'var(--text-base)', color: 'var(--text-heading)' }}>{r.v}</div>
                  </div>
                </div>
            )}
            </Card>
            <Card>
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                    ...
                    <span style={{ flex: 1, fontSize: 'var(--text-base)', color: 'var(--text-body)' }}>{h.label}</span>
            ...
            style={{ width: '100%', minHeight: 100, padding: 10, fontFamily: 'var(--font-sans)', fontSize: 'var(--text-base)',
    ...
    return (
      <div style={{ alignSelf: me ? 'flex-end' : 'flex-start', maxWidth: '78%', display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ padding: '9px 14px', borderRadius: 'var(--radius-md)',
          ...
          fontSize: 'var(--text-base)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{m.text}</div>
  ...
  function SuggestionCard({ s, onApprove, onDismiss, onEditDraft }) {
    ...
    return (
      <Card style={{ borderColor: 'color-mix(in srgb, var(--brand) 18%, var(--border-default))' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
              <AiOrb size={16} />
              <span style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--text-heading)' }}>{s.trigger}</span>
            </div>
        ...
        style={{ width: '100%', minHeight: 100, padding: 11, fontFamily: 'var(--font-sans)', fontSize: 'var(--text-base)',
        ...
        <div style={{ padding: 12, background: 'var(--surface-sunken)', borderRadius: 'var(--radius-md)',
          fontSize: 'var(--text-base)', color: 'var(--text-body)', lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>{draft}</div>
        }
      ...
      <div className="ps-fade" style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%', maxWidth: 760, margin: '0 auto', width: '100%' }}>
        <div ref={scrollerRef} style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14, padding: 6 }}>
          ...
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              color: 'var(--text-body)', lineHeight: 1.6, whiteSpace: 'pre-wrap', fontSize: "14px" }}>{m.content}</div>
              </div> :
